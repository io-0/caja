...
class CacheTestAsync {
  ...
  void cachedKeysShouldBeRetrievable() {
    ...
    var firstAKeys = aCaches.stream().map(c -> await(c.keys())).collect(Collectors.toList());
    var firstBKeys = bCaches.stream().map(c -> await(c.keys())).collect(Collectors.toList());
    var firstCKeys = cCaches.stream().map(c -> await(c.keys())).collect(Collectors.toList());
    var secondAKeys = aCaches.stream().map(c -> await(c.keys())).collect(Collectors.toList());
    var secondBKeys = bCaches.stream().map(c -> await(c.keys())).collect(Collectors.toList());
    var secondCKeys = cCaches.stream().map(c -> await(c.keys())).collect(Collectors.toList());
    org.awaitility.Awaitility.await().pollInterval(2*1000 + 200, TimeUnit.MILLISECONDS).until(() -> true);
    var thirdAKeys = aCaches.stream().map(c -> await(c.keys())).collect(Collectors.toList());
    var thirdBKeys = bCaches.stream().map(c -> await(c.keys())).collect(Collectors.toList());
    var thirdCKeys = cCaches.stream().map(c -> await(c.keys())).collect(Collectors.toList());
  ...
  void cachesShouldBeClearable() {
    ...
    bCaches.forEach(c -> await(c.clear()));
    bCaches.forEach(c -> await(c.clear()));
    cCaches.forEach(c -> await(c.clear()));
    cCaches.forEach(c -> await(c.clear()));
  ...
  void cachesShouldWorkWithEmptyContext() {
    // Given a cache, data and keys
    Cache<String, Integer> cache = cacheManager1.getAsAsync(CACHE_A, Context.of(), String.class, Integer.class);

    // When the data is cached
    await(cache.put(oneKey1, oneValue1));

    // Then the data should be retrievable
    assertTrue(await(cache.containsKey(oneKey1)));
    assertEquals(oneValue1, await(cache.get(oneKey1)));
  }
  ...
  private ComplexKey complexKey1 = new ComplexKey("k1", 1, new Nested(true, List.of(1, 2, 3)));
  private ComplexValue complexValue1 = new ComplexValue(1L, BigDecimal.TEN, LocalDateTime.now(), new Nested(true, List.of(10, 20, 30)));
  private ComplexKey complexKey2 = new ComplexKey("k2", 2, new Nested(true, List.of(2, 4, 6)));
  ...
  private <K, V, W> List<Cache<K, V>> setupCaches(String name, Class<K> keyType, Class<V> valueType, Class<W> valueSubType, CacheManager... managers) {
    return Arrays.stream(managers)
      .map(m -> m.getAsAsync(name, keyType, valueType, valueSubType))
      .collect(Collectors.toList());
  }
  ...
  private <K, V> List<Cache<K, V>> setupCaches(String name, Class<K> keyType, Class<V> valueType, Class<?> valueSubType1, Class<?> valueSubType2, CacheManager... managers) {
    return Arrays.stream(managers)
      .map(m -> m.getAsAsync(name, keyType, valueType, valueSubType1, valueSubType2))
      .collect(Collectors.toList());
  }
  ...
  private <K, V> void assertKeysPresent(List<Cache<K, V>> caches, List<K> keys) {
    caches.forEach(c -> keys.forEach(k -> assertTrue(await(c.containsKey(k)), format("Cache %d", caches.indexOf(c)+1))));
  }

  private <K, V> void assertKeysAbsent(List<Cache<K, V>> caches, List<K> keys) {
    caches.forEach(c -> keys.forEach(k -> assertFalse(await(c.containsKey(k)), format("Cache %d", caches.indexOf(c)+1))));
  }

  private <K, V> void assertValuesPresent(List<Cache<K, V>> caches, Map<K, V> data) {
    caches.forEach(c -> data.forEach((k, v) -> assertEquals(v, await(c.get(k)), format("Cache %d", caches.indexOf(c)+1))));
  }

  private <K, V> void assertValuesAbsent(List<Cache<K, V>> caches, List<K> keys) {
    caches.forEach(c -> keys.forEach(k -> assertNull(await(c.get(k)), format("Cache %d", caches.indexOf(c)+1))));
  }
  ...
  private <K, V> void assertGetTroughFillsCache(List<Cache<K, V>> caches, Map<K, V> data) {
    caches.forEach(c -> data.forEach((k, v) -> assertEquals(v, await(c.getThrough(k, () -> v)), format("Cache %d", caches.indexOf(c)+1))));
  }

  private <K, V> void assertGetTroughUsesCache(List<Cache<K, V>> caches, Map<K, V> data) {
    caches.forEach(c -> data.forEach((k, v) -> assertEquals(v, await(c.getThrough(k, Assertions::fail)), format("Cache %d", caches.indexOf(c)+1))));
  }
  ...
  private <K, V> void assertCacheRefDoesntMatter(List<Cache<K, V>> aCaches, List<Cache<K, V>> bCaches, List<K> keys) {
    IntStream.range(0, aCaches.size()).forEach(i ->
      keys.forEach(k -> assertEquals(await(aCaches.get(i).get(k)), await(bCaches.get(i).get(k)), format("Cache %d", i+1)))
    );
  }
